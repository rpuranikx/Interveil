...
export function checkCommand(command: string, userConfig: UserCommandConfig = {}): MatchResult {
  const cmd = command.toLowerCase();

  // Whitelist overrides everything
  for (const pattern of userConfig.whitelist ?? []) {
    if (matchPattern(cmd, pattern)) {
      return { matched: false, rule: pattern, source: 'whitelisted' };
    }
  }
  ...
  for (const pattern of userConfig.requireApproval ?? []) {
    if (matchPattern(cmd, pattern)) {
      return { matched: true, rule: pattern, source: 'approval_required' };
    }
  }
  ...
  for (const pattern of userConfig.block ?? []) {
    if (matchPattern(cmd, pattern)) {
      return { matched: true, rule: pattern, source: 'user_config' };
    }
  }
  ...
  for (const pattern of defaultPatterns) {
    if (matchPattern(cmd, pattern)) {
      return { matched: true, rule: String(pattern), source: 'default_blocklist' };
    }
  }
